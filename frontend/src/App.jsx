          ...
          {result && (
            <>
              <div className="prediction-main">
                ...
                <h3>{result.prediction.display_class || result.prediction.predicted_class}</h3>
                <p className="confidence">
                  Confidence: {(result.prediction.confidence * 100).toFixed(2)}%
                </p>
                <p className={`confidence-warning ${result.prediction.confidence_info?.level}`}>
                  {result.prediction.confidence_info?.message}
                </p>
                {result.prediction.validation && (
                  <p className={`validation-message ${result.prediction.validation.status}`}>
                    {result.prediction.validation.message}
                  </p>
                )}
              </div>
              ...
              <div className="top-list">
                ...
                {result.prediction.top_predictions.map((item) => (
                  <div className="top-item" key={item.rank}>
                    <span>
                      #{item.rank} {item.display_name || item.class_name}
                    </span>
